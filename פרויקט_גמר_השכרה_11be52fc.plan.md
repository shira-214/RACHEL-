...
> **📌 הערה ליום 2 — מחיקה עם Foreign Key (משימה 7):**
> מחיקת רשומה **עלולה להיכשל** אם יש רשומות תלויות ב-DB. זו התנהגות תקינה — לא באג.
>
> | מוחקים | חסום אם קיים |
> |--------|----------------|
> | דירה (Apartment) | הזמנות, תמונות, תוספות-דירה |
> | שוכר (Hirer) | הזמנות |
> | עיר / רחוב / אזור | דירות, קישורי AraesCitiesStreet |
> | תוספת (Extra) | ExtrasApartement |
>
> **מה לעשות ב-WPF (שלב 2.5):** בכל כפתור "מחק" ב-`All*` pages:
> 1. לבדוק את `bool` שחוזר מ-WCF (`Delete*` מחזיר `false` = נכשל)
> 2. להציג `MessageBox` בעברית, למשל: *"לא ניתן למחוק — קיימות רשומות מקושרות (הזמנות/תמונות). מחק קודם את התלויות."*
> 3. לא להציג Exception גולמי למשתמש
>
> **סדר מחיקה מומלץ להדגמה:** קודם הזמנות → תמונות → תוספות-דירה → דירה
...
- **מחיקה + FK:** ראה הערה מתחת למשימה 7 — MessageBox בעברית כש-`Delete*` מחזיר `false`
